...
8. O Gate 3 é desligado por padrão e exige flag global mais allowlists explícitas de workspaces e flows. Fora dessa combinação, inclusive no card legado, o comportamento continua marker-only; o reconciliador não percorre schemas fora da allowlist de workspaces.
9. No flow habilitado, a mesma transação que bloqueia a sessão persiste em `workflow_v2.dialer_supplier_v2` uma intenção com sessão, revisão fixada, card, lista, membro e `dial_profile_id`. A chave idempotente é determinística sobre essa identidade.
