...
> **ARQUIVADO / NAO IMPLEMENTAR COMO API DO ORCH.** Em 2026-09-24, a Metrics
> API assumiu persistencia, consultas, agregacoes e UI. Este documento
> preserva semantica, filtros e casos de aceite como insumo para adaptar a
> emissao de eventos do ORCH quando o contrato oficial da Metrics for
> recebido. Rotas e envelopes abaixo nao estao aprovados para implementacao.
...
- `person_uuid` opcional e usado como identidade de filtro. Nome,
  identificador ou canal sao pesquisados no autocomplete existente do Target
  Core/BFF, que resolve a pessoa antes da chamada ao relatorio;
...
- payloads e metadados livres nao sao persistidos na action; novos atributos
  escalares exigem extensao explicita do contrato;
