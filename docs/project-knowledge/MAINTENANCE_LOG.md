...
- A PR ORCH `#154` integrou o commit funcional `a27e110`; o merge `ed3d84f` foi implantado em `10.1.20.136` e `10.1.20.237`. Os `.env` foram preservados nos backups `.maintenance-backups/.env.pre-select-contact-channel-engine-ed3d84f`; a API do `136` e as 21 units ORCH ativas do `237` foram reiniciadas conforme a topologia encontrada. Ambos terminaram em `ed3d84f`, com API ativa e `/health/celery=200`.
- Somente depois do rollout ORCH, o flow canário foi adicionado à `ORCHESTRATOR_PERSON_SCOPE_FLOW_UUIDS` dos Target Core `10.1.20.128`, `10.1.20.129`, `10.1.20.239` e `10.1.20.249`. Em cada host, o `.env` anterior ficou em `.maintenance-backups/.env.pre-person-c114383d-20260907`; a comparação neutralizando a linha confirmou que nenhuma outra configuração mudou.
- A decisão da allowlist ocorre em `FlowV2Service`, nos perfis de API `full` e `crud`. Esses dois serviços foram reiniciados de forma rolling primeiro no `239` e depois no `249`; os quatro processos carregaram os dois UUIDs da allowlist, responderam health `200` e não registraram erro no journal. Os workers de sessões do `128`/`129` permaneceram ativos e não foram reiniciados, pois recebem o lote já decidido e não consultam a allowlist. A implementação da flag estava presente nas duas revisões Target observadas (`ec04338` nos workers e `7234fed` nas APIs), sem atualização Git adicional neste rollout.
- O canário E2E desvinculou e vinculou novamente pela rota oficial `POST /v2/flow/{flow_uuid}/mailings` o mailing `d7ebf802-babc-40eb-9172-9417091deaed`, com duas pessoas. As duas operações retornaram HTTP `200`, sem erro, e o mailing terminou vinculado. O desvínculo preservou o histórico e marcou as sessões anteriores como desatribuídas (`state=5`).
