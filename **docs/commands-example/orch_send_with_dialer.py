...
class OrchSendWithDialerHandler(CommandHandler):
...
def _execute_blocking_send(
    *, actuator_value: str, flow, state: Dict[str, Any], cmd, helpers: Dict[str, Any]
    ...
    logger = logging.getLogger("target_core.orch_send_with_dialer")
    logger.info(
        "orch_send_with_dialer.start",
        extra={"workspace_uuid": workspace_uuid, "member_id": member_id, "contact": contact},
    )
    ...
    if (not member_id) and workspace_uuid:
        ...
        try:
            ...
            logging.getLogger("target_core.orch_send_with_dialer").debug(
                "orch_send_with_dialer.fallback_fetch_failed",
                exc_info=True,
    ...
    if workspace_uuid and member_id:
        ...
        # Debug defensivo: sem member_id não persistimos.
        logger.debug(
            "orch_send_with_dialer.skip_update",
            extra={"workspace_uuid": workspace_uuid, "member_id": member_id, "next_ref": next_ref},
        )
    logger.info(
        "orch_send_with_dialer.exit",
        extra={"workspace_uuid": workspace_uuid, "member_id": member_id, "next_ref": next_ref},
    )

    # Bloqueante: interrompe loop após este componente.
