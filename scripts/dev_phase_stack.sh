...
SWITCH_BOT_FLOW_QUEUE="${CELERY_SWITCH_BOT_FLOW_QUEUE:-orch_switch_bot_flow_f5_local}"
...
start_proc() {
  ...
  (
    cd "${ROOT_DIR}"
    nohup bash -lc "source '${VENV_ACTIVATE}' && exec env ${cmd}" >"${log_file}" 2>&1 &
    echo $! >"${pid_file}"
  )
...
CELERY_SWITCH_BOT_FLOW_QUEUE=${SWITCH_BOT_FLOW_QUEUE}
...
start_all() {
  show_env
  start_proc "api" \
    "ORCH_QUEUE_PROFILE=f5_local CELERY_DISPATCH_QUEUE=${DISPATCH_QUEUE} CELERY_EXECUTE_QUEUE=${EXECUTE_QUEUE} CELERY_SWITCH_BOT_FLOW_QUEUE=${SWITCH_BOT_FLOW_QUEUE} CELERY_DIALER_SUPPLIER_V2_QUEUE=${DIALER_SUPPLIER_V2_QUEUE} CELERY_HEARTBEAT_QUEUE=${HEARTBEAT_QUEUE} CELERY_S3_FILES_INGEST_QUEUE=${FILEAPP_INGEST_QUEUE} CELERY_SOURCE_LIST_INGEST_QUEUE=${FILEAPP_PROCESS_QUEUE} CELERY_FILEAPP_MAILING_ASSOC_QUEUE=${FILEAPP_MAILING_ASSOC_QUEUE} uvicorn app.main:app --host ${API_HOST} --port ${API_PORT}"
  wait_for_api
...
CELERY_DISPATCH_QUEUE=${DISPATCH_QUEUE} CELERY_EXECUTE_QUEUE=${EXECUTE_QUEUE} CELERY_SWITCH_BOT_FLOW_QUEUE=${SWITCH_BOT_FLOW_QUEUE} CELERY_DIALER_SUPPLIER_V2_QUEUE=${DIALER_SUPPLIER_V2_QUEUE} CELERY_HEARTBEAT_QUEUE=${HEARTBEAT_QUEUE} \
...
celery -A app.core.celery_app:celery_app worker --hostname=orch-celery-worker@_macbook_deivid_dev -n orch-worker-legacy@%h -Q ${DISPATCH_QUEUE},${EXECUTE_QUEUE},${SWITCH_BOT_FLOW_QUEUE},${DIALER_SUPPLIER_V2_QUEUE},${HEARTBEAT_QUEUE} --without-mingle --without-gossip -l INFO"
