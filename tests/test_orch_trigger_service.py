...
@pytest.mark.asyncio
async def test_process_single_payload_tabulacao_updates_existing_session_by_address(monkeypatch) -> None:
    monkeypatch.setattr(
        orch_trigger_service,
        "extract_session_fields",
        lambda app_name, payload: SessionExtraction(
            ...
            entity_type="person",
            entity_address="5511975620806",
            entity_session_id="GW02-1781218669.3313",
        ),
    )
    ...
    async def _fail_if_called(*args, **kwargs):  # type: ignore[no-untyped-def]
        raise AssertionError("persist_session não deveria ser chamado para tabulação do run_flow")
    ...
    assert response.accepted is True
    assert response.session_id == 6650
    assert response.session_created is False


@pytest.mark.asyncio
async def test_process_single_payload_tabulacao_resumes_recent_session_by_run_flow(monkeypatch) -> None:
    captured: dict[str, str | int] = {}
    ...
    monkeypatch.setattr(
        ...
        lambda app_name, payload: SessionExtraction(
            ...
            entity_type="person",
            entity_address="5511975620806",
            entity_session_id="GW02-1781218669.3314",
        ),
    )
    ...
    monkeypatch.setattr(
        orch_trigger_service,
        "get_settings",
        lambda: SimpleNamespace(workflow_dialer_event_correlation_window_hours=24),
    )
    monkeypatch.setattr(orch_trigger_service, "persist_session", _fail_if_called)
    monkeypatch.setattr(orch_trigger_service, "persist_channel_events", _fake_persist_channel_events)
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_process_single_payload_tabulacao_ignores_when_session_not_found(monkeypatch) -> None:
    captured: dict[str, str] = {}
    ...
    monkeypatch.setattr(
        ...
        "extract_session_fields",
        lambda app_name, payload: SessionExtraction(
            entity="dialer-action-6",
            entity_type="person",
            entity_address="5511975620806",
            entity_session_id="GW02-1781218669.3315",
        ),
    )
    ...
    assert response.accepted is False
    assert response.status == "ignored"
    assert captured["reason"] == "run_flow_tabulacao_session_not_found_by_address"
