...
EMAIL_REF = "11111111-1111-1111-1111-111111111111"
...
def _component() -> dict:
    return {
        "ref_id": EMAIL_REF,
        "component_id": "send_with_email",
        "parameters": {
            "from_email": "atendimento@example.com",
            "subject_template": "Assunto sigiloso {{contact.full_name}}",
            "email_format": "html",
            "html_body_template": "<p>Conteúdo sigiloso {{contact.full_name}}</p>",
            "completion_event": "delivered",
...
def _runtime(*, session_scope: str = "channel", member_id: int = 77) -> dict:
    return {
        ...
        "workflow_v2": {
            ...
            "next_card_cursor": EMAIL_REF,
        },
        "variables": {"payload": {}, "customs": {}},
    }


def _contact_row(*, member_id: int = 77, channel_type: str = "email") -> dict:
    return {
        "contact_list_member_id": member_id,
        "contact_list_id": CONTACT_LIST_UUID,
        "mailing_id": "1140",
        ...
        "contact_channel_label": "email_1",
        "contact_channel_address": "person@example.test",
...
def _configure_execution(
    ...
    *,
    ...
    session_state = {
        "uuid": SESSION_UUID,
        "state": 0,
        "entity_address": "person@example.test",
        "runtime_variables": runtime,
        "last_card_uuid": None,
        "next_card_uuid": EMAIL_REF,
        "frozen_until": None,
    }
...
@pytest.mark.asyncio
async def test_channel_email_marks_exact_member_and_blocks_without_http(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _runtime()
    ...
    http_execute = Mock(side_effect=AssertionError("email marker-only cannot call HTTP"))
    monkeypatch.setattr(workflow, "assign_email_routing_for_session", assign_email)
    ...
    assert result.stopped_reason == "blocked_send_with_email"
    assert result.executed_steps == 1
    assert result.last_card_uuid == EMAIL_REF
    assert result.next_card_uuid is None
    assign_email.assert_awaited_once()
    http_execute.assert_not_called()
    assert persisted[-1]["last_card_uuid"] == EMAIL_REF
    assert persisted[-1]["next_card_uuid"] is None
    assert runtime["workflow_v2"]["blocking_stop_reason"] == "blocked_send_with_email"
    assert runtime["send_with_email_routing"]["assignment"]["linked_actuator"] == "email"
    assert "Assunto sigiloso" not in str(runtime)
    assert "Conteúdo sigiloso" not in str(runtime)
    assert "atendimento@example.com" not in str(runtime)


@pytest.mark.asyncio
async def test_person_email_requires_and_uses_explicitly_selected_email_member(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    _configure_execution(
        ...
        contact_row={
            **_contact_row(member_id=88),
            "contact_channel_address": "other@example.test",
        },
    )
    assign_email = AsyncMock(
        return_value={
    ...
    assert result.stopped_reason == "blocked_send_with_email"
    assert assign_email.await_args.kwargs["contact_list_member_id"] == 88
    assert assign_email.await_args.kwargs["person_uuid"] == PERSON_UUID


@pytest.mark.asyncio
async def test_person_email_without_selection_terminalizes_without_marking(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _runtime(session_scope="person")
    persisted = _configure_execution(
    ...
    assign_email.assert_not_awaited()


@pytest.mark.asyncio
@pytest.mark.parametrize("channel_type", ["voice", "phone", "sms", "whatsapp", "rcs"])
async def test_non_email_member_is_not_eligible(
    monkeypatch: pytest.MonkeyPatch,
    channel_type: str,
) -> None:
    runtime = _runtime()
    ...
    assert result.stopped_reason == "send_with_email_contact_not_eligible"
    assert persisted[-1]["state"] == 3
    assign_email.assert_not_awaited()


@pytest.mark.asyncio
async def test_blocked_email_reentry_does_not_mark_again(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _runtime()
    runtime["workflow_v2"].update(
        {
            ...
            "blocking_stop_reason": "blocked_send_with_email",
            "last_card_cursor": EMAIL_REF,
    ...
    _configure_execution(
        ...
        session_state_overrides={"last_card_uuid": EMAIL_REF, "next_card_uuid": None},
    )
    assign_email = AsyncMock()
    monkeypatch.setattr(workflow, "assign_email_routing_for_session", assign_email)
...
def test_email_block_dispatcher_and_terminal_alarm_are_registered() -> None:
    assert workflow._blocking_stop_reason_for_component("send_with_email") == (
        "blocked_send_with_email"
    )
    assert "blocked_send_with_email" in workflow_dispatcher_service.BLOCKING_RUNNING_STOP_REASONS
    assert "send_with_email_contact_not_eligible" in (
        workflow_dispatcher_service.TERMINAL_FAILURE_STOP_REASONS
    )
    assert m2_alarm_from_stopped_reason("send_with_email_contact_not_eligible") == (
        "error",
        "workflow_m2_send_with_email_contact_not_eligible",
