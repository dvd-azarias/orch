...
@pytest.mark.asyncio
async def test_assign_email_is_guarded_by_exact_active_session_and_email_member() -> None:
    session = _RecordingSession(
        {"id": 77, "linked_actuator": "email", "previous_linked_actuator": None}
    )
    ...
    assert "= 'email'" in session.statement
    assert "FOR UPDATE OF clm, os" in session.statement
    assert "linked_actuator = 'email'" in session.statement
    assert session.parameters == BASE


@pytest.mark.asyncio
async def test_assign_email_reports_already_marked_without_exposing_payload() -> None:
    session = _RecordingSession(
        {"id": 77, "linked_actuator": "email", "previous_linked_actuator": "email"}
    )
...
@pytest.mark.asyncio
async def test_assign_email_returns_none_when_guarded_update_matches_nothing() -> None:
