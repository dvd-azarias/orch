...
@pytest.mark.asyncio
async def test_send_with_email_isolated_marker_guards_and_rollback() -> None:
    flow_uuid = str(uuid4())
        ...
        async with db_session.begin():
            ...
            with pytest.raises(RuntimeError, match="force outer workflow failure"):
                async with db_session.begin_nested():
                    marked_before_failure = await assign_email_routing_for_session(
                        db_session, **common
                    )
            ...
            marked = await assign_email_routing_for_session(db_session, **common)
            assert marked == {
                "contact_list_member_id": 77,
                "linked_actuator": "email",
                "mode": "marked",
            }
            marked_again = await assign_email_routing_for_session(db_session, **common)
            assert marked_again == {
                "contact_list_member_id": 77,
                "linked_actuator": "email",
                "mode": "already_marked",
            ...
            voice_member = await assign_email_routing_for_session(
                db_session,
                **{**common, "contact_list_member_id": 88},
            )
            wrong_person = await assign_email_routing_for_session(
                db_session,
                **{**common, "person_uuid": str(uuid4())},
            )
            wrong_mailing = await assign_email_routing_for_session(
                db_session,
