...
@pytest.mark.asyncio
async def test_send_with_rcs_isolated_marker_guards_and_rollback() -> None:
    flow_uuid = str(uuid4())
        ...
        async with db_session.begin():
            ...
            await db_session.execute(
                text(
                    ...
                    INSERT INTO contact_list_members (
                        ...
                        (
                            77, '12345678901', 'rcs', '5511999990001',
            ...
            with pytest.raises(RuntimeError, match="force outer workflow failure"):
                async with db_session.begin_nested():
                    marked_before_failure = await assign_rcs_routing_for_session(
                        db_session, **common
                    )
            ...
            marked = await assign_rcs_routing_for_session(db_session, **common)
            assert marked == {
                "contact_list_member_id": 77,
                "linked_actuator": "rcs",
                "mode": "marked",
            }
            marked_again = await assign_rcs_routing_for_session(db_session, **common)
            assert marked_again == {
                "contact_list_member_id": 77,
                "linked_actuator": "rcs",
                "mode": "already_marked",
            ...
            voice_member = await assign_rcs_routing_for_session(
                db_session,
                **{**common, "contact_list_member_id": 88},
            )
            wrong_person = await assign_rcs_routing_for_session(
                db_session,
                **{**common, "person_uuid": str(uuid4())},
            )
            wrong_mailing = await assign_rcs_routing_for_session(
                db_session,
