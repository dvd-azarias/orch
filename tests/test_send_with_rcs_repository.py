...
@pytest.mark.asyncio
async def test_assign_rcs_is_guarded_by_exact_active_session_and_rcs_member() -> None:
    session = _RecordingSession(
        {"id": 77, "linked_actuator": "rcs", "previous_linked_actuator": None}
    )
...
@pytest.mark.asyncio
async def test_assign_rcs_reports_already_marked_without_exposing_payload() -> None:
    session = _RecordingSession(
        {"id": 77, "linked_actuator": "rcs", "previous_linked_actuator": "rcs"}
    )
...
@pytest.mark.asyncio
async def test_assign_rcs_returns_none_when_guarded_update_matches_nothing() -> None:
