...
@pytest.mark.asyncio
async def test_delivery_receipt_alone_is_not_ingest_evidence() -> None:
    session = SimpleNamespace(execute=AsyncMock(return_value=_FirstResult(None)))

    result = await tasks._has_fileapp_ingest_evidence(
        session,
        workspace_schema="ws_test",
        file_id="8ea2b45d-85e6-442c-b365-9355ae2cc2b8",
        original_name="critical.csv",
        monitored_folder="monitoramento/upload",
    )

    assert result is False
    query = str(session.execute.await_args.args[0])
    assert "source_lists" in query
    assert "arquivos_s3_events" not in query
...
@pytest.mark.asyncio
async def test_reconcile_entrada_rescue_skips_when_flow_is_in_flight(monkeypatch) -> None:
    def _fake_get_session_factory():
        return lambda: _DummySession()
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=[
                {
                    "id": "file-3",
                    "original_name": "carga-repetida.csv",
                    "folder_path": "ACAN_CONTATOS/entrada",
    ...
    monkeypatch.setattr(tasks, "_has_fileapp_ingest_evidence", AsyncMock(return_value=False))
    monkeypatch.setattr(tasks, "_try_acquire_fileapp_entrada_rescue_file_lock", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(tasks, "_get_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: 1)
    monkeypatch.setattr(tasks, "_set_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(tasks, "_get_fileapp_entrada_rescue_flow_state", lambda *_args, **_kwargs: "in_flight")
    monkeypatch.setattr(tasks, "resolve_mapping_template_uuid", AsyncMock(return_value="tmpl-1"))
    monkeypatch.setattr(tasks, "persist_alarm", AsyncMock())
    monkeypatch.setattr(tasks, "quarantine_file_to_falha", AsyncMock())
    monkeypatch.setattr(
        "app.tasks.fileapp_ingest_tasks.ingest_fileapp_tipo1_event_task.apply_async",
        lambda **_kwargs: (_ for _ in ()).throw(AssertionError("apply_async não deveria ser chamado")),
    )
...
@pytest.mark.asyncio
async def test_reconcile_entrada_rescue_scans_past_evidence_without_consuming_action_batch(monkeypatch) -> None:
    captured: list[str] = []
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            ...
            celery_fileapp_entrada_rescue_max_retries=3,
            celery_fileapp_entrada_rescue_lock_seconds=120,
            celery_fileapp_entrada_rescue_batch_size=2,
            arquivos_base_url="https://sync-core-api.otima.io/files/v1",
            sync_ws_timeout_seconds=5,
            target_core_api_bearer_token=None,
            arquivos_client_id="cid",
    ...
    monkeypatch.setattr(
        tasks,
        "_fetch_fileapp_rescue_flow_targets",
        AsyncMock(return_value=[{"flow_uuid": "flow-1", "monitored_folders": ["monitoramento/upload"]}]),
    )
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            return_value=[
                {
                    "id": f"file-{index}",
                    "original_name": f"file-{index}.csv",
                    "folder_path": "monitoramento/upload",
                    "url": f"https://files/{index}",
                    "created_at": (old + timedelta(seconds=index)).isoformat(),
                }
                for index in range(1, 5)
            ]
        ),
    )
    monkeypatch.setattr(tasks, "_has_fileapp_ingest_evidence", AsyncMock(side_effect=[True, True, False, False]))
    monkeypatch.setattr(tasks, "_try_acquire_fileapp_entrada_rescue_file_lock", lambda *_args, **_kwargs: True)
    monkeypatch.setattr(tasks, "_get_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: 0)
    monkeypatch.setattr(tasks, "_set_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(tasks, "resolve_mapping_template_uuid", AsyncMock(return_value="tmpl-1"))
    ...
    monkeypatch.setattr(
        "app.tasks.fileapp_ingest_tasks.ingest_fileapp_tipo1_event_task.apply_async",
        lambda **kwargs: captured.append(kwargs["kwargs"]["payload"]["file"]["id"]) or SimpleNamespace(id="task-3"),
    )

    result = await tasks._reconcile_fileapp_entrada_rescue_task()

    assert result["files_scanned"] == 4
    assert result["skipped_evidence"] == 2
    assert result["reingested"] == 2
    assert captured == ["file-3", "file-4"]


@pytest.mark.asyncio
async def test_reconcile_entrada_rescue_durable_claim_overrides_stale_redis_state(monkeypatch) -> None:
    file_id = "8ea2b45d-85e6-442c-b365-9355ae2cc2b8"
    flow_state_updates: list[str] = []
    ...
    monkeypatch.setattr(
        tasks,
        "_fetch_fileapp_rescue_flow_targets",
        AsyncMock(return_value=[{"flow_uuid": "706c6fef-85f2-4276-bcfd-eb28f75acde2", "monitored_folders": ["monitoramento/upload"]}]),
    )
    monkeypatch.setattr(
        tasks,
        "_list_files_in_folder",
    ...
    monkeypatch.setattr(tasks, "_get_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: 0)
    monkeypatch.setattr(tasks, "_set_fileapp_entrada_rescue_attempts", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(tasks, "_get_fileapp_entrada_rescue_flow_state", lambda *_args, **_kwargs: "done")
    monkeypatch.setattr(
        tasks,
        "_set_fileapp_entrada_rescue_flow_state",
        lambda *_args, **kwargs: flow_state_updates.append(kwargs["state"]),
    )
    monkeypatch.setattr(tasks, "resolve_mapping_template_uuid", AsyncMock(return_value="tmpl-1"))
    monkeypatch.setattr(
        tasks,
        "_claim_fileapp_ingest_receipt_for_worker",
        AsyncMock(return_value={"id": 77, "status": "accepted", "task_id": None, "should_enqueue": True}),
    )
    monkeypatch.setattr(tasks, "_mark_fileapp_ingest_receipt_enqueued", mark_enqueued)
    monkeypatch.setattr(tasks, "persist_alarm", AsyncMock())
    monkeypatch.setattr(tasks, "quarantine_file_to_falha", AsyncMock())
    monkeypatch.setattr(
        "app.tasks.fileapp_ingest_tasks.ingest_fileapp_tipo1_event_task.apply_async",
        lambda **_kwargs: SimpleNamespace(id="task-77"),
    )

    result = await tasks._reconcile_fileapp_entrada_rescue_task()

    assert result["reingested"] == 1
    assert flow_state_updates == ["in_flight"]
    mark_enqueued.assert_awaited_once_with(workspace_uuid="w1", receipt_id=77, task_id="task-77")
