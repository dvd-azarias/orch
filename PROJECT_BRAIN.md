...
17. O card `select_contact_channel` da PR ORCH `#154` foi implantado em `10.1.20.136` e `10.1.20.237`. Depois do deploy, o flow `c114383d-72e1-4401-8877-765e5bfac27f` entrou na allowlist `person` do Target Core nos quatro hosts. O vínculo oficial de um mailing com duas pessoas gerou exatamente as sessões `7513`/`7514`: ambas selecionaram o canal `voice` primário, terminaram em `state=3`, sem erro, e produziram os POSTs externos `1351296`/`1351297` confirmados no `api-bin`.
   ...
   - Complemento pós-atendimento: `flow_override` só pode retomar o mesmo telefone de um `answered` quando um `wait_for_event` ativado pelo mesmo card consumiu uma tabulação nova e correlacionada. O ORCH pede autorização à rota Supplier V2 dedicada, aceita somente o membro corrente e, após sucesso, arquiva o terminal antigo e devolve o registro a `ready`; o Dialer fica bloqueado até uma nova tentativa/terminal. O canvas decide a jornada, mas calendário e limites continuam sendo reavaliados pela Supplier. Códigos como CPC/RECADO/IMPRODUTIVA não são hardcoded.
18. `send_with_sms` preserva o handoff marker-only quando o Gate de canal V2 está desligado: a engine grava `linked_actuator=sms` no membro exato e bloqueia a sessão. Sob `CHANNEL_SUPPLIER_V2_ENABLED` e allowlists de workspace/flow, o mesmo commit persiste uma intenção cifrada e idempotente pinada à sessão, revisão, card, lista, membro, canal e sequência; somente depois do commit uma fila exclusiva a registra em `POST /v2/contact-supplier/channel-dispatches`. O ORCH nunca chama o provedor. O Target revalida a revisão executável, o card, o membro e o marcador antes do outbox e expõe seu estado durável em uma leitura interna sanitizada. Com `CHANNEL_SUPPLIER_V2_CALLBACKS_ENABLED`, o card expõe somente `Próximo`, gera internamente DLR/MO/status e publica `customs.<output_var>.correlation_key`. O reconciliador libera o card somente ao observar `accepted`; a transição idempotente agenda uma única retomada. DLR/MO/status permanecem eventos `callback/sms_event` exclusivos do `wait_for_event` seguinte, com `data.status=sent|delivered|not_delivered|response|failed|status|dlr`; eles não são mais pré-condição para concluir o envio. Um wait subsequente pode filtrar a chave exata; se uma condição retornar ao mesmo wait, o prazo absoluto original é reutilizado. Espera sem correlação conserva o contrato genérico anterior e continua proibida em flows `channel`; espera correlacionada é permitida. Callback histórico é auditado sem reabrir a sessão. RCS e Supplier V1 permanecem inalterados. Deploy/canário permanecem pendentes. Consulte R33 e `CHANNEL_DISPATCH_V2_PLAN.md`.
...
- `send_with_sms` / `send_with_rcs`: marcam o membro exato e bloqueiam a sessão. Sob Gate/allowlists exclusivos, persistem envelope cifrado na mesma transação e o registram pós-commit no outbox da Supplier V2. Para SMS, o estado durável `accepted`, consultado pelo reconciliador, conclui somente o card de envio; DLR/MO/status seguem pelo ledger e pelo `wait_for_event`. RCS continua dependendo de sua branch de ciclo de vida/timeout.
...
- Em 2026-09-22, o job `generate_file` do mesmo flow tinha 212 linhas pendentes e intervalo físico médio de 21,94s: lock global por job, transação longa, processamento sequencial e `listdir` integral por arquivo anulavam a concorrência. A correção usa claim unitário/lock por alvo, `stat` SFTP e dez faixas de dreno; o FileApp ganhou retries curtos no mesmo mailing. O SLO real depende do canário pós-rollout.
...
- `docs/project-knowledge/ORCHESTRATION_REPORTING_PLAN.md` — fonte unica da verdade para a telemetria de jornadas enviada pelo ORCH a Metrics. O plano original de API/BFF/UI propria foi substituido em 2026-09-25 por fatos incrementais e snapshots via WebSocket/SYNC, sem responsabilidade do PDIAL. O funil usa as sete etapas dos cards, a cobertura comeca em marco zero por workspace, nao existe backfill e o retorno ao canario/fluxo completo permanece obrigatorio.
- `docs/project-knowledge/ORCHESTRATION_JOURNEY_METRICS_WS_CONTRACT.md` — proposta versionada do contrato ORCH -> Metrics, incluindo transporte SYNC, fatos, snapshots, funil, saude, canais, privacidade e as decisoes ainda pendentes de ACK/sala/limites; fixtures em `ORCHESTRATION_JOURNEY_METRICS_WS_EXAMPLES.json`.
