...
14. `source_list_membership`, exibido como **Gerenciar Contato na Lista**, exige estado explícito e pertence a flows `person`. `mailing_source=session_origin` resolve a lista pelo membro, flow e vínculo ativo que originaram a própria sessão; `selected` preserva a seleção explícita anterior. Definições antigas sem esse campo continuam equivalentes a `selected`. `active` cria/reutiliza o draft e reativa membros já materializados somente no flow atual; `inactive` preserva os dados, marca esses membros com `unassigned_at`, limpa roteamento e encerra apenas sessões irmãs ativas da mesma pessoa/lista/flow. A sessão corrente conclui a branch; nenhuma ação associa mailing ao flow, materializa membro ausente ou cria sessão.
...
- `send_with_sms` / `send_with_rcs`: marcam o membro exato e bloqueiam a sessão. Sob Gate/allowlists exclusivos, persistem envelope cifrado na mesma transação e o registram pós-commit no outbox da Supplier V2; aceite do provedor não retoma o flow.
