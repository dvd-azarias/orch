...
20. O endpoint manual do Target Core preserva `entity_session_id=entity_address:::flow_uuid`, mas, quando recebe explicitamente `session_scope=channel`, usa também o `contact_list_member_id` como identidade de reuso. Sem isso, canais distintos do mesmo contato/endereço podiam colapsar na sessão ativa mais recente. A correção sem migration persiste a identidade em `runtime_variables.session_identity`, aceita o payload histórico como fallback e não altera callback, unassign ou chamadas sem escopo; implantação e novo canário do flow RCS ainda estão pendentes. Consulte R35.
