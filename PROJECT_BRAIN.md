...
18. `send_with_sms` está implementado localmente no branch `feat/send-with-sms-runtime` como handoff marker-only, sem envio HTTP pelo ORCH: a engine exige o membro SMS exato, grava `linked_actuator=sms` e bloqueia a sessão na mesma transação. Testes focados, PostgreSQL temporário com rollback forçado e smoke da stack passaram; canários `channel`/`person`, integração da PR e rollout permanecem pendentes. A marca expressa intenção de roteamento, mas não é um envelope pronto para envio. Antes de ativar SMS real, o ORCH deve materializar um contrato seguro e idempotente com mensagem renderizada, revisão fixada, parâmetros do provedor, callbacks e credencial protegida ou sua referência. Supplier/Target não pode carregar nem reinterpretar o grafo do flow para reconstruir esse conteúdo. Consulte o item 6 do roadmap e o risco R33.
